...
# TO IMPLEMENT:use the config file to change the heuristic method for A*
#HEURISTIC_METHOD = rospy.get_param('~astar/heuristic/method', 'euclidean')
# # #{ class Node
class Node:
    ...
    def pos(self):
        return self.__pos
    def __heuristicFunction(self, method:str = 'euclidean'):
        a = self.pos[0] - self.goal[0]
        b = self.pos[1] - self.goal[1]
        c = self.pos[2] - self.goal[2]
        if method == 'euclidean':
            return distEuclidean(self.pos, self.goal)
        elif method == 'manhattan':
            return sum([abs(a), abs(b), abs(c)])
        else:
            raise NotImplementedError("Heuristic function guiding the state space exploration not specified. Select in ['euclidean', 'manhattan'].")
# # #}
# # #{ class AStar
class AStar():
    ...
    def getPathDistance(self, path):
        ...
        return distance
    def halveAndTest(self, path, path_distance, ratio:float = 0.5):
        pt1 = path[0][0:3]
        pt2 = path[-1][0:3]
        if len(path) <= 2:
        ...
        while dist_subpath < path_distance*ratio:
            for i in range(1, len(path)):
                subpath = path[0:i]
                dist_subpath = self.getPathDistance(subpath)
        if self.grid.obstacleBetween(pt1, pt2):
            seg1 = path[:i]
            seg2 = path[i:]
            seg1.extend(seg2)
            return seg1
        return [path[0], path[i], path[-1]]
    def generatePath(self, m_start, m_goal):
        ...
        if node is None:
            ...
            path.append(start)
            if self.straighten:
                path_distance = self.getPathDistance(path)
                path = self.halveAndTest(path, path_distance)
            path.reverse()
            for node in path:
