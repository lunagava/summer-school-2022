...
class TrajectoryUtils():
    ...
    def interpolateHeading(self, waypoints):
        ...
        while idx != (len(waypoints) - 1):
            ...
            for i in range(1, len(subtraj) - 1):
                ...
                # get segment points and distance
                subtraj_0 = subtraj[i - 1].point
                subtraj_1 = subtraj[i].point
                dist_01 = distEuclidean(subtraj_0, subtraj_1)

                # get linear interpolation as function of relative distance between two points
                hdg_interp = hdg_from + (dist_01/subtraj_len)*d_hdg
                hdg_interp = unwrapAngle(hdg_interp)
    ...
    def computeTimeParametrization(self, waypoints, v_lims, a_lims):
        ...
        gridpoints = np.linspace(0, path.duration, 15*len(waypoints))
    ...
    def resolveCollisions(self, method, problem, trajectories, safety_distance):
        ...
        ## |  [COLLISION AVOIDANCE METHOD #2]: Delay UAV with shorter trajectory at start until there is no collision occurring  |
            ...
            #raise NotImplementedError('[STUDENTS] Collision prevention method \'delay_till_no_collisions_occur\' not finished. You have to finish it on your own.')
            # Tips:
